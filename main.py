...
if __name__ == '__main__':
    ...
    ''' optional: Load '''
    # model = load_model('model(48, 48, 96)_shape(384, 384, 3)_dropout1_40e.h5')
    # print("Model Loaded!")
    ''' optional: Return Training'''
    # history = LossHistory()
    # model.fit(train_x, train_y, epochs=epochs, batch_size=batch_size, verbose=2, shuffle=True, callbacks=[history])
    # np.savetxt('loss_history20.txt', np.array(history.losses), delimiter=',')
    ''' Save '''
    # model.save('model(10, 10, 10, 10, 96)_shape(384, 384, 3).h5', True)
    # model.save('model(48, 48, 48, 48, 96)_shape(384, 384, 3)_40e.h5', True)
    # model.save('model(48, 48, 48, 48, 96)_shape(384, 384, 3)_60e.h5', True)
    model.save('model(48, 48, 96)_shape(384, 384, 3)_dropout1_20e.h5', True)
    print("Model Saved!")
